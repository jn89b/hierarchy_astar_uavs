# -*- coding: utf-8 -*-
"""
Created on Wed Dec  1 11:46:03 2021
...
def spawn_uavs(home_position_list, loiter_location_list):
    """spawn drones"""
    uavs_list = []
    for idx, home_position in enumerate(home_position_list):
        uav_id = "uav"+str(idx)
        uav = Drone.QuadCopter(uav_id, home_position, loiter_location_list[idx], loiter_location_list[idx], True)
...
def run_utm(n_simulations, n_drones, min_h, max_h):
    """begin simulation"""
    performance = []
    ...
    for i in range(0,n_simulations):
        ...
        uavs = spawn_uavs(home_position_list, loiter_location_list)
        overallDb = UTMDatabase.OverallDatabase()
        #commented out random uavs here
        overall_db = overallDb.listen_for_incoming_uavs(uavs)    
        homeBase = HomeBase()
...
if __name__ == '__main__':    
    ...
    for i in range(min_uavs, max_uavs+1):
        performance, db, n_drones, results = run_utm(n_simulations, i, min_h, max_h)
